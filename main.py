...
def align_data_to_target_axis(
    ...
    """
    将源数据 (source_data) 映射到 目标轴 (target_axis) 上。
    未匹配到的源数据(如同位素)将被丢弃。
    
    Args:
        source_data: (H, W, C) 或 (C, H, W) 或 (C,) 的 numpy array
        source_axis: (C_source,) 对应的源 m/z 轴 (例如 final_axis / axis_iso)
        target_axis: (C_target,) 目标 m/z 轴 (例如 slim0_axis / axis_noiso)
        ppm_tol: 匹配容差 (PPM)
        fill_value: 如果目标轴的某个点在源轴里找不到对应，填什么值 (默认0)
        
    Returns:
        target_data: 形状与 source_data 对应，但 C 维变成了 len(target_axis)
    """
    
    # 1. 维度检查与标准化
    # 我们统一把 Channel 维放到最后处理，最后再转回去
    input_is_chw = False
    ...
    for i, target_mz in enumerate(target_axis):
        ...
        # 检查 idx 和 idx-1 两个邻居
        candidates = []
        if idx < len(source_axis): candidates.append(idx)
...
def independent_isotope_removal_debug(data_cube, mz_axis, ppm_tol=20.0, max_isotope_level=2):
    ...
    for i in range(C):
        ...
        ratios = []
        ratios.append(min(p * 1.3,0.45))          # M+1 ratio (index 0)
        ratios.append(min(0.45 * p**2 * 1.3,0.15)) # M+2 ratio (index 1)
        ...
        for k in range(1, max_isotope_level + 1):
            ...
            candidates = []
            if idx < C: candidates.append(idx)
            if idx > 0: candidates.append(idx - 1)
            ...
            # 情况 B: 找到了 (执行减法)
            # 获取当前 k 对应的 ratio
            # k=1 -> ratio[0], k=2 -> ratio[1]
            current_ratio = ratios[k-1] if (k-1) < len(ratios) else 0.0
            
            # 只有当确实要执行非零减法时才打印
            # (如果 M+k 本来就是 0，减了也没意义，就不打印了，干扰视线)
...
def plot_deconvolution_results(model, img_shape, mz_axis, component_names=None):
    """
    可视化函数：展示每个解卷积出来的组分的空间分布和对应的纯净谱图。
    
    参数:
    - model: 训练好的 LipidDeconvolution 对象
    - img_shape: 元组 (height, width)，用于还原空间图像
    - mz_axis: 列表或数组，对应 m/z 的数值
    - component_names: 组分名称列表 (可选)
    """
    C = model.C_
    S = model.S_
    n_comps = C.shape[1]
    ...
    for i in range(n_comps):
        ...
        # 2. 右图：解卷积谱图 (Deconvolved Spectrum)
        ax_spec = plt.subplot(gs[i, 1])
        spectrum = S[i, :]
        
        # 绘制针状图 (Stem plot)
        markerline, stemlines, baseline = ax_spec.stem(mz_axis, spectrum, linefmt='b-', markerfmt='bo', basefmt=" ")
        plt.setp(stemlines, 'linewidth', 1)
        plt.setp(markerline, 'markersize', 3)
        
        ax_spec.set_title(f"Deconvolved MS/MS Spectrum", fontsize=10)
        ax_spec.set_ylabel("Intensity")
        if i == n_comps - 1:
            ax_spec.set_xlabel("m/z")
        
        # 标注强峰
        top_indices = np.argsort(spectrum)[-15:] # 标出最强的3个峰
        for idx in top_indices:
            if spectrum[idx] > 0.1 * np.max(spectrum): # 只有显著的峰才标
                ax_spec.text(mz_axis[idx], spectrum[idx], f"{mz_axis[idx]:.2f}", 
...
# clean_data = projection_isotope_removal_3d(
#     my_data_cube, 
#     torch.Tensor(fragment_mzs),
#     fwhm_func_left, 
#     fwhm_func_right, 
#     subtraction_weight=0.9, # 建议保守一点，防止扣多了
#     sigma_cutoff=3.0        # 3倍 Sigma 覆盖绝大部分峰面积
# )
...
data_log = (torch.Tensor(cleaned_data))/np.sum(cleaned_data,axis=2,keepdims=True)*1000#/(torch.norm(torch.Tensor(cleaned_data), dim=1, keepdim=True)+0.00001)*1000
# data_log = (torch.Tensor(cleaned_data))/(torch.sum(torch.Tensor(cleaned_data), dim=1, keepdim=True)+0.00001)*1000
...
alpha=0.001 #(稍微给一点稀疏惩罚，把底噪压为0)
smooth_strength=3 #(开启3x3区域的平滑，强力镇压噪点)
real_mask = spec[:, kept_].T # 确保形状是 (73, 19)
...
def smart_select_pixels(data_matrix, n_select=5000, n_components=50):
    ...
    优先选择“化学成分独特”的像素，保证矩阵满秩，防止漏掉稀有异构体。
    """
    ...
    selected_indices = np.random.choice(
        data_matrix.shape[0], size=n_select, replace=False, p=probs
    )
    return selected_indices

def get_adaptive_cutoff(contributions):
    """
    自适应寻找 Major/Minor 的分割点 (拐点法 / Elbow Method)。
    """
    n = len(contributions)
    if n < 2: return 0
    ...
    cumsum_norm = cumsum / cumsum[-1] # 归一化到 0-1
...
def visualize_clusters_2d(labels, H, W, fore_mask, n_clusters):
    """
    【可视化核心函数】将聚类标签映射回 2D 空间并绘图
    """
    # 1. 创建一张全 NaN 的空图
    cluster_map_1d = np.full(H * W, np.nan)
    
    # 2. 将聚类标签填入前景位置 (注意：fore_mask 必须是布尔值或索引)
    # 如果 fore 是布尔矩阵 (H, W)，先展平
    if fore_mask.ndim == 2:
        fore_flat = fore_mask.flatten()
        cluster_map_1d[fore_flat] = labels
    # 如果 fore 是索引数组 (indices)
    else:
    ...
    # 选择离散色盘 (Discrete Colormap)
    if n_clusters <= 10:
        cmap = plt.get_cmap('tab10', n_clusters)
    else:
        cmap = plt.get_cmap('viridis', n_clusters)
        
    cmap.set_bad(color='black') # 背景设为黑色
    
    # 使用 masked_array 处理 NaN 值，确保背景正确显示
...
class SegmentedUnmixer:
    ...
    def _apply_top_k_truncation(self, coefs, k):
        if np.count_nonzero(coefs) <= k: return coefs
    ...
    def _core_solve_routine(self, X_input, y_input, alpha, l1_ratio, top_k):
        model = ElasticNet(alpha=alpha, l1_ratio=l1_ratio, 
                           ...
                           max_iter=50000, copy_X=False, tol=1e-9)
    ...
    def solve(self, Y_sub):
        ...
        if np.sum(valid_cols) == 0: return W_out
        ...
        for frag_idx in target_indices:
            if frag_idx in forbidden_frag_indices: continue
    ...
    def solve_peeling(self, Y_sub, major_indices, minor_indices):
        ...
        for frag_idx in target_indices:
            ...
            if frag_mz <= 500: alpha, l1, k = 1e-5, 0.9, 2
            elif frag_mz <= 650: alpha, l1, k = 1e-5, 0.9, 2
            else: alpha, l1, k = 1e-5, 0.9, 2
            ...
            if np.max(y_resid_input) < 1e-9: continue 
...
# 假设变量已经存在: H, W, fore, data_scaled, mz_slim, pre
# 如果没有，请取消下面这几行的注释并替换为你的真实数据
# H, W = 100, 100
# fore = np.random.rand(H*W) > 0.5
# data_scaled = np.random.rand(np.sum(fore), 1000)
# mz_slim = np.linspace(100, 1000, 1000)
# pre = np.linspace(200, 900, 50)
pre=[770.570894,758.489]
data_fore = data_scaled[fore, :]
row_sums = np.sum(data_fore, axis=1, keepdims=True)
row_sums[row_sums == 0] = 1.0
# data_fore = data_fore / row_sums * 1000.0
# 初始化
...
print("\n=== Step 2: 聚类 (含可视化) ===")
n_clusters = 4
top_feat = np.argsort(np.sum(data_fore, axis=0))[-300:] # 只用信号最强的前300个特征聚类
...
try:
    ...
    # 使用 visualize_clusters_2d 而不是 visualize_clusters
...
# ... (前面的 Step 1, Step 2 保持不变) ...

# =========================================================
# 修正后的 Step 3: 联合求解 (Joint Solving)
# =========================================================
print("\n=== [Fixed] Step 3: 联合求解 (Joint Solving) ===")
results_dict = {}

# 1. 修正前体列表精度问题 (防止找不到列索引)
# 务必保证 solver 里的 precursor_mzs 和你数据的精度一致
# 建议这里重新初始化一下 solver，确保万无一失
pre_array = np.sort(np.unique(np.array(pre))) # 你的输入 pre
...
for k in range(n_clusters):
    ...
    # 智能采样：不用选太少，选个 100-200 个点能算得更准
    if Y_sub.shape[0] > 200:
        idx = smart_select_pixels(Y_sub, n_select=50)
        Y_sub = Y_sub[idx, :]
    
    # 【核心修改】直接用 solve()，不要用 solve_peeling()！
    # solve() 会让 PC 和 PS 放在同一个方程里公平竞争权重
...
def find_lipid_candidates(precursor_mz, mz_axis, spectrum, database, offset_dict=None, ppm_tol=20, top_k=1):
    """ 支持 Offset (源内裂解) 的鉴定逻辑 """
    if offset_dict is None: offset_dict = {}
    ...
    if candidates_df.empty: return [], offset_used
    
    max_int = np.max(spectrum)
    if max_int < 1e-9: return [], offset_used
    ...
    for _, row in candidates_df.iterrows():
        std_frags = row['msms_m0_noiso'] # 已经是列表格式
        if not std_frags: continue
        ...
        if len(current_indices) >= 2: # 至少匹配2个碎片
            # 简单的评分：匹配数量
...
# =========================================================
# Part B: Solver 组件 (Handler & Projector)
# =========================================================
...
offset_config = {770.570894: 50.0, 671.466: 87.03} # 根据需要修改

if 'pre' in globals(): target_precursor_mzs = np.unique(np.around(pre, 3))
else: target_precursor_mzs = np.unique(np.around(list(results_dict.keys())[0]['pmz'], 3))
...
for k, W_matrix in tqdm(results_dict.items(), desc="Analyzing Clusters"):
    ...
    for idx in active_indices:
        try: pmz_obs = target_precursor_mzs[idx]
        except: continue
        
        spectrum = W_matrix[idx]
        current_total_int = np.sum(spectrum)
        
        candidates, offset_used = find_lipid_candidates(pmz_obs, mz_slim, spectrum, custom_db, offset_dict=offset_config)
        
        for cand in candidates:
            name = cand['name']
            if offset_used > 0: name += f" [ISD+{offset_used:.0f}]"
            ...
            if temp_frag_dict:
                ...
                if name not in best_lipid_data or current_total_int > best_lipid_data[name]['max_int']:
                    best_lipid_data[name] = {'max_int': current_total_int, 'spectrum': norm_frags, 'pmz': pmz_obs}
...
all_frags = set()
for uid in lipid_id_list: all_frags.update(best_lipid_data[uid]['spectrum'].keys())
...
for i, uid in enumerate(lipid_id_list):
    for mz_k, val in best_lipid_data[uid]['spectrum'].items():
        ...
        if np.abs(final_mz_arr[b_idx] - mz_k) < 0.05: H_fixed[i, b_idx] = val
...
for i, uid in enumerate(tqdm(lipid_id_list, desc="Exporting")):
    ...
    save_robust_image(img_flat_pre.reshape(handler.H, handler.W), os.path.join(l_dir, "Precursor_Raw.png"), cmap_name='gray')
